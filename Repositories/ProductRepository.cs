...
namespace backend.Repositories
{
	public class ProductRepository : IRepository<Product>
	{
		...
		{
			string query = @"SELECT ProductID, Name, Type, Description, ImageURL FROM Products";
		...
		{
			string query = @"SELECT ProductID, Name, Type, Description, ImageURL FROM dbo.PRODUCT WHERE ProductID = @ProductID";
			...
			try
			{
				using (SqlConnection myCon = new SqlConnection(_connectionString))
				{
					...
					using (SqlCommand myCommand = new SqlCommand(query, myCon))
					{
						...
						myCommand.Parameters.AddWithValue("@Type", product.Type);
		...
		public IEnumerable<Product> GetById2(int id)
		{
			throw new NotImplementedException();
		}

		public bool Add2(Product item, int id)
		{
			throw new NotImplementedException();
		}
